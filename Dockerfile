...
ENV     RUNDECK_VERSION=3.0.18-20190322
ENV     RDECK_BASE=/etc/rundeck
ENV     RDECK_JAR=$RDECK_BASE/app.jar
ENV     PATH=$PATH:$RDECK_BASE/tools/bin

ADD     http://dl.bintray.com/rundeck/rundeck-maven/rundeck-${RUNDECK_VERSION}.war $RDECK_JAR
RUN     apk add --update openjdk8-jre bash curl ca-certificates openssh-client nss && \
        mkdir -p $RDECK_BASE && \
        rm -Rf /var/cache/apk/*
